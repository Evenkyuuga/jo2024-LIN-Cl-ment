...
<body>
    ...
    <main>
        ...
        <div class="action-buttons">
            <button onclick="openAddEventForm()">Ajouter un Calendrier</button>
            <!-- Autres boutons... -->
        </div>
        <!-- Tableau des sports -->
        <?php
        ...
        try {
            ...
            if ($statement->rowCount() > 0) {
                ...
                echo "<th class='couleur'>nom_epreuve</td>";
                echo "<th class='couleur'>date_epreuve</td>";
                echo "<th class='couleur'>heure_epreuve</td>";
                ...
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo "<td class='color'>" . htmlspecialchars($row['nom_epreuve']) . "</td>"; // Utilisation de la classe 'color' pour changer la couleur
                    echo "<td class='color'>" . htmlspecialchars($row['date_epreuve']) . "</td>"; // Utilisation de la classe 'color' pour changer la couleur
                    echo "<td class='color'>" . htmlspecialchars($row['heure_epreuve']) . "</td>"; // Utilisation de la classe 'color' pour changer la couleur
                    echo "<td class='color'>" . htmlspecialchars($row['id_lieu']) . "</td>"; // Utilisation de la classe 'color' pour changer la couleur
                    echo "<td class='color'>" . htmlspecialchars($row['id_sport']) . "</td>"; // Utilisation de la classe 'color' pour changer la couleur
                    echo "<td><button onclick='openModifyPlacesForm(\"{$row['id_epreuve']}\", \"{$row['nom_epreuve']}\", \"{$row['date_epreuve']}, \"{$row['heure_epreuve']}, \"{$row['id_lieu']}, {$row['id_sport']}\")'>Modifier</button></td>";
                    echo "<td><button onclick='deletePlacesConfirmation(\"{$row['id_epreuve']}\", \"{$row['nom_epreuve']}\", \"{$row['date_epreuve']}, {$row['heure_epreuve']}, \"{$row['id_lieu']}, {$row['id_sport']}\")'>Supprimer</button></td>";
                    echo "</tr>";
                }
...
// (fonctionne à condition d’avoir activé l’option en local)
    ...
    <script>
        ...
        function openModifyEventForm(id_sport) {
        ...
        function deleteEventConfirmation(id_sport) {
